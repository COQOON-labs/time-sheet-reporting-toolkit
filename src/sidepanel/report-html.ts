...
import { escapeHtml, fmtHours, fmtOvertime } from '../lib/format.js';
...
export function buildReportHtml(opts: {
  ...
  printHelperUrl?: string;
}): string {
  const { range, filters, entries, overtimeMinutes, printHelperUrl } = opts;
  ...
  const otText = fmtOvertime(overtimeMinutes);
...
<style>
  * { box-sizing: border-box; }
  ...
  .print-hint button { margin-left: 10px; padding: 4px 10px; cursor: pointer; }
  @media print { .print-hint { display: none; } body { margin: 16px; } }
</style>
</head><body>
  <div class="print-hint">
    <b>Tipp:</b> Cmd/Ctrl+P → Ziel auf "Save as PDF" → Speichern.
    <button id="print-btn">🖨 Print</button>
  </div>
  <header>
    <h1>Personio Time Report</h1>
  ...
  <div class="summary">
    ...
    <div class="kpi"><span>Overtime</span><strong>${escapeHtml(otText)}</strong></div>
  ...
  ${printHelperUrl ? `<script src="${escapeHtml(printHelperUrl)}"></script>` : ''}
</body></html>`;
}
